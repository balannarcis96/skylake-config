...
template <CConfigTargetType _Object, typename _ProxyType, CConfigTargetType _TargetConfig, CContainerType _Container>
    requires(config::CConfigProxyType<_Object, _ProxyType>)
class ArrayViaProxyField : public ConfigField<_TargetConfig> {
public:
    ...
    ArrayViaProxyField(Field* f_parent, std::string_view f_field_name, member_ptr_t f_member_ptr, ConfigNode<_ProxyType>&& f_proxy_config) noexcept
        : ConfigField<_TargetConfig>(f_parent, f_field_name)
        , m_member_ptr(f_member_ptr)
        , m_config(std::move(f_proxy_config)) {
        m_config.update_parent(*this);
    }

    ~ArrayViaProxyField() override                               = default;
    ArrayViaProxyField(const ArrayViaProxyField&)                = default;
    ArrayViaProxyField& operator=(const ArrayViaProxyField&)     = default;
    ArrayViaProxyField(ArrayViaProxyField&&) noexcept            = default;
    ArrayViaProxyField& operator=(ArrayViaProxyField&&) noexcept = default;

    ArrayViaProxyField& required(bool f_required) noexcept {
        m_required = f_required;
        return *this;
    }

    ArrayViaProxyField& default_value(std::vector<_Object>&& f_default) {
        m_default = std::move(f_default);
        return *this;
    }

    ArrayViaProxyField& min_length(u32 f_min_length) noexcept {
        m_min_length = f_min_length;
        return *this;
    }

    ArrayViaProxyField& max_length(u32 f_max_length) noexcept {
        m_max_length = f_max_length;
        return *this;
    }

    ArrayViaProxyField& min_max_length(u32 f_min_length, u32 f_max_length) noexcept {
        m_min_length = f_min_length;
        m_max_length = f_max_length;
        return *this;
    }
    ...
    ArrayViaProxyField& truncate_on_overflow(bool f_truncate_on_overflow) noexcept
...
private:
    ...
    void validate() override {
        if (m_is_default) {
            ...
            for (const auto& field : m_default.value()) {
                m_entries.push_back(m_config);
                _ProxyType temp{};
                if (false == temp.load(m_entries.back(), field)) {
                    throw std::runtime_error("Proxy array -> proxy failed to load from object!");
                }
                m_entries.back().load_fields_from_default_object(temp);
            }
        }
    ...
    void load_value_from_default_object(const _TargetConfig& f_config) override {
        ...
        for (const auto& entry : field) {
            m_entries.push_back(m_config);
            _ProxyType temp{};
            if (false == temp.load(m_entries.back(), entry)) {
                throw std::runtime_error("Proxy array -> proxy failed to load from object!");
            }
            m_entries.back().load_fields_from_default_object(temp);
        }
    ...
    void load_value_for_validation_only(const _TargetConfig& f_config) override {
        ...
        for (const auto& entry : field) {
            ...
            m_entries.back().load_fields_for_validation_only(temp);
        }
    ...
    std::unique_ptr<ConfigField<_TargetConfig>> clone() override {
        return std::make_unique<ArrayViaProxyField<_Object, _ProxyType, _TargetConfig, _Container>>(*this);
    }
...
private:
    ...
    ConfigNode<_ProxyType>              m_config;
    std::vector<ConfigNode<_ProxyType>> m_entries;
